...
候选归因（未定论）：e39 达到全程峰值后，val 曲线**有明显波动但未恢复该
峰值**（如 e48–e51 从 0.2979→0.3124、e68–e69 从 0.3064→0.3161 等 19 次向上
波动），e80 比峰值低 0.0201；这与后期泛化退化一致，但**不能单凭该曲线定因
为过拟合**。residual 尺度漂移（proj norm e39→e80 继续 +22~25%）为伴随现象；
corruption 强度冻结无变化。（更正 2026-08-17：上一版"单调下降/过拟合为主"的
表述不准确。）
