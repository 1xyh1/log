...
def main() -> None:
    ...
    p.add_argument("--run-name", default="F1C-I-magsoft")
    p.add_argument("--fixed-run", default="F1C-I-fixed")
    p.add_argument("--original-run", default="F1C-I-soft")
    ...
    report = {
        "schema": "step4-f1-c-ir-quality-probe-v1",
    ...
    try:
        ...
        for kind, severity in conditions:
            ...
            original_ap = evu.evaluate_dataset_stock_semantics(
                original_model, dataset, device, names)
            ...
            report["conditions"][key] = {
                ...
                "original_gate_soft": original_ap,
                "learned_minus_original_soft_map50_95": (
                    learned["map50_95"] - original_ap["map50_95"]),
                "scan_overrides": scan,
    ...
    report["interpretation_inputs"] = {
        ...
        "adaptive_gain_interpretation": (
            ...
            "result is CONSTANT ATTENUATION DOMINATES, ADAPTIVITY IS "
            "NOT PROVEN — q x P(A) scale coupling makes the raw q magnitude "
